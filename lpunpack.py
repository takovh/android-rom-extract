...
from simg2img import SparseImage
    ...
    def unpack(self):
        try:
            if SparseImage(self._fd).check():
                ...
                in_path = Path(self._fd.name)
                unsparse_path = in_path.parent / f'{in_path.stem}.unsparse.img'
                SparseImage(self._fd).unsparse(unsparse_path)
                self._fd.close()
                self._fd = open(str(unsparse_path), 'rb')
                print('[ok]')
